...
// Obtenemos todos los datos que necesitamos del texto introducido por el usuario
    ...
    var input_ciudad=valor_textoArea.match(/\w{2,}(?=\)\:.*\d{2}\:\d{2})/g)
...
this.fechaSalida=valor_textoArea.match(/(?<=Ida\:\s{0,})\d{2}\/\d{2}\/\d{4}/g);//(comentario=developer(contnuarporaqui))
this.ciudadSalida=input_ciudad[0]
this.ciudadLlegada=input_ciudad[1]
...
//CREAMOS FORMATOS DE FECHAS VALIDOS PARA QUE AMADEUS LO ACEPTE
                                ...
                                input_ciudad[0]=input_ciudad[0].replace(/^\n{0,}|\n{0,}$/,"");
                                input_ciudad[0]=input_ciudad[0].replace(/\n{0,}$/,"");
                                input_ciudad[1]=input_ciudad[1].replace(/^\n{0,}|\n{0,}$/,"");
                                input_ciudad[1]=input_ciudad[1].replace(/\n{0,}$/,"");
